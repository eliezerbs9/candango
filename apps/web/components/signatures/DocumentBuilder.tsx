...
import { Anchor, Badge, Button, Center, FileButton, Group, Loader, Paper, Select, Stack, Switch, Text, TextInput, Tooltip } from '@mantine/core';
...
import { hasSigningField, type SignableDocMode } from '@/lib/api/signable-documents';
...
async function pdfToPageImages(file: File): Promise<Blob[]> {
  ...
  // Serve the worker from our own origin (copied into public/ by scripts/copy-pdf-worker.mjs) — a
  // cross-origin CDN module worker is unreliable and silently hangs getDocument().
  pdfjs.GlobalWorkerOptions.workerSrc = '/pdf.worker.min.mjs';
  // Guard against a stuck worker: never let the spinner hang forever — fail loudly after 30s.
  const loading = pdfjs.getDocument({ data: await file.arrayBuffer() });
  const doc = await Promise.race([
    loading.promise,
    new Promise<never>((_, reject) => setTimeout(() => { loading.destroy().catch(() => {}); reject(new Error('Reading the PDF timed out. Please try again.')); }, 30000)),
  ]);
  const blobs: Blob[] = [];
...
export function DocumentBuilder({ id }: { id: string }) {
  // useSearchParams() (read below) must sit under a Suspense boundary for `next build` to prerender.
  return (
    <Suspense fallback={<Center mih="40vh"><Loader /></Center>}>
      <DocumentBuilderInner id={id} />
    </Suspense>
  );
}

function DocumentBuilderInner({ id }: { id: string }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const importKey = searchParams.get('importPdf'); // a deal PDF queued to become the page backgrounds
  ...
  // Opened from a deal with ?importPdf=<key>: download that deal PDF (through our own API, no CORS) and
  // rasterize it into page backgrounds, once. The builder stays usable even if this fails.
  const importDoneRef = useRef(false);
  useEffect(() => {
    if (!hydrated || !importKey || importDoneRef.current) return;
    importDoneRef.current = true;
    (async () => {
      try {
        const blob = await apiFetchBlob(`/uploads/download?key=${encodeURIComponent(importKey)}`, { token: token ?? undefined });
        await importPdf(new File([blob], 'document.pdf', { type: 'application/pdf' }));
      } catch {
        notifications.show({ message: 'Could not import the PDF automatically — use “Import PDF” to add it.', color: 'yellow' });
      } finally {
        router.replace(pathname); // drop the ?importPdf param so a refresh won't re-import
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hydrated, importKey, token]);
  ...
  const canSend = hasSigningField(pages); // a document with no signature/initials field can't be sent
    ...
    <Stack gap="md">
      ...
      <Group justify="space-between" align="flex-end" wrap="wrap">
        ...
        <Group gap="sm">
          ...
          {doc?.dealId && (
            <Tooltip label="Add at least one signature or initials field before sending." disabled={canSend} withArrow>
              <Button leftSection={<IconSend size={14} />} onClick={sendCtl.open} disabled={!canSend}>
                Send for signature
              </Button>
            </Tooltip>
          )}
      ...
      {doc?.dealId && (
        <SendDocModal opened={sendOpen} onClose={sendCtl.close} dealId={doc.dealId} docId={id} secondSigner={parties === 'both'} />
      )}
      ...
      <Paper withBorder radius="md" p="md">
        <Group align="flex-end" gap="lg" wrap="wrap">
          ...
          <div>
            ...
            <Group gap="md" align="center" h={36}>
              <Text size="sm" c="dimmed">
                Customer <Text span c="dimmed">(always)</Text>
              </Text>
              <Switch
                label="Second signer (your side)"
                checked={parties === 'both'}
                onChange={(e) => {
                  if (e.currentTarget.checked) {
                    setParties('both');
                    setParty2Source('owner');
                  } else {
                    setParties('one');
